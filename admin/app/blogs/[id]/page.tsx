...
import { getBlog, getUsers, getCategories, updateBlog, Blog, User, Category } from "@/lib/api";
...
export default function BlogEditorPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [blogData, usersData, categoriesData] = await Promise.all([
          getBlog(blogId),
          getUsers(),
          getCategories()
        ]);
        ...
        if (blogData) {
          setFormData({
            ...
            author_ids: blogData.authors?.map((a) => a.id) || [],
            category_ids: blogData.categories?.map((c) => c.id) || [],
          });
        }
      } catch (error) {
  ...
  // Custom code block style (black & white)
  ...
  if (!blog) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center">
        <p className="text-black/40 mb-4">Blog nicht gefunden</p>
        <Link href="/blogs" className="text-black underline">Zurück zur Übersicht</Link>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[24px] font-semibold tracking-tight text-black">Blog bearbeiten</h1>
            <p className="text-[13px] text-black/40 font-mono mt-0.5">{blogId}</p>
          </div>
        </div>
        <button
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    type="button"
                    onClick={() => insertMarkdown("[Link](url)", false)}
                    className="w-8 h-8 rounded-lg hover:bg-black/[0.04] flex items-center justify-center transition-colors"
                    title="Link"
                  >
            ...
            <div className="max-h-[calc(100vh-200px)] overflow-y-auto">
              ...
              <div className="relative h-48 bg-black/[0.04]">
                {(imageFile || blog?.image) ? (
                  <Image
                    src={imageFile ? URL.createObjectURL(imageFile) : blog?.image || ""}
                    alt="Preview"
                    fill
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="pt-4 border-t border-black/[0.06]">
                  {formData.content ? (
                    ...
                    <p className="text-[14px] text-black/30 italic">
                      Content eingeben um Vorschau zu sehen...
                    </p>
                  )}
                </div>
              </div>
